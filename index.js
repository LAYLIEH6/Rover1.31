(async () => {
        ...
        if (((s4dmessage.channel).id) == '1152696923602559016') {
            if (((s4dmessage.content) || '').startsWith('r-help' || '')) {
              ...
              **/item info**
                ...
                s4d.client.channels.cache.get('1152696923602559016').send({
                    embeds: [w]
                });
            }
        }
    ...
    s4d.client.on('messageCreate', async (s4dmessage) => {
        ...
        if (((s4dmessage.channel).id) == '1150838224873341069') {
            s4d.client.channels.cache.get('1150838224873341069').messages.fetch({
                limit: 5
            ...
            s4d.client.channels.cache.get('1150838224873341069').send({
                embeds: [Verify]
            });
        }
    ...
    s4d.client.on('messageCreate', async (s4dmessage) => {
        ...
        if (((s4dmessage.channel).id) == '1150544561991590058') {
            ...
            **11. 点这里看中文版的规则： [未知行星-服务器的规则](https://docs.google.com/document/d/19EZOQZ1_hKcTenXpV4dwZ5iuJx5ZvT2KdkM_SeoqJl8/edit?usp=sharing)**
            ...
            s4d.client.channels.cache.get('1150544561991590058').send({
                embeds: [Rules]
            });
        }
    ...
    s4d.client.on('guildMemberAdd', async (param1) => {
        ...
        s4d.client.channels.cache.get('1150982448939810878').send({
            embeds: [Welcome]
        });
    ...
    s4d.client.on('messageCreate', async (s4dmessage) => {
        if ((s4dmessage.content) == '-deduction progess') {
            if ((s4dmessage.channel) == s4d.client.channels.cache.get('1152696923602559016')) {
                const key = String(s4dmessage.author)
                console.log(key);

                if (!deduction.has(key)) {
                    deduction.set(key, 0);
                }

                s4dmessage.channel.send({
                    content: String((String(`Deduction Progress: `) + String(deduction.get(key))))
                })
            }
        }
    ...
    s4d.client.on('messageCreate', async (s4dmessage) => {
        if (((s4dmessage.content) || '').startsWith('-pts add' || '')) {
            if ((s4dmessage.author)._roles.includes(((s4d.client.guilds.cache.get('1150544148181549116')).roles.cache.get('1150556182893826048')).id)) {
                ...
                const key = String(s4dmessage.mentions.members.first())
                console.log(key);
